...
const CalendarLights = ({ helloesData, helloesDataSorted, earliestDataPoint, latestDataPoint, lightUpList }) => {
  ...
  const renderDay = (item, lightUp, rowStart, weekData) => {
    ...
    if (!item) {
        return(
            <View
            style={[
              styles.daySquare, { backgroundColor: 'transparent', opacity: 0 }  
    ...
    if (!lightUp) {
        return(
            <View
            style={[
              styles.daySquare, { backgroundColor: 'transparent' }  
    ...
    return (
      <View style={styles.calendarContainer}>
        ...
        <View style={styles.innerCalendarContainer}>
          {renderWeeks(item.monthData.daysInMonth, indexRangeStart, item.helloData.days)}
        </View> 
      </View>
    );
  };
  ...
  useEffect(() => {
    if (monthsData && helloesDataSorted) {
      ...
      const combined = monthsData.map((month) => {
        // Match by index
        const helloData = helloesDataSorted.find(
