...
const AlertProfileBottomUser = ({ visible, profileData, onClose }) => {
    ...
    const [firstName, setFirstName] = useState(authUserState.user.profile.first_name);
    ...
    const handleDeleteAddress = async (title) => {
        try {
            ...
            setUserAddresses(userAddresses.filter(address => address.title !== title));
        } catch (error) {
                    ...
                    <ScrollView contentContainerStyle={styles.scrollContainer}>
                        ...
                        <View>
                            <View style={styles.row}>
                                ...
                                <Text style={styles.modalTitle}>{authUserState.user.username}</Text>
                            </View>
                            <View style={styles.row}>
                            ...
                            <View style={styles.addressRow}>
                                ...
                                {userAddresses.map((addressData, index) => (
                                    <View key={index} style={styles.addressSection}>
                                        <ButtonAddress address={addressData} onDelete={handleDeleteAddress} />
                                    </View>
                                ))}
                            ...
                            <View style={styles.row}>
                                <Text>High Contrast Mode: {authUserState.user.settings.high_contrast_mode ? 'Enabled' : 'Disabled'}</Text>
                            </View>
                            <View style={styles.row}>
                                <Text>Large Text: {authUserState.user.settings.large_text ? 'Enabled' : 'Disabled'}</Text>
                            </View>
                            <View style={styles.row}>
                                <Text>Receive Notifications: {authUserState.user.settings.receive_notifications ? 'Enabled' : 'Disabled'}</Text>
                            </View>
                            <View style={styles.row}>
                                <Text>Screen Reader: {authUserState.user.settings.screen_reader ? 'Enabled' : 'Disabled'}</Text>
                            </View>
                            <TouchableOpacity onPress={toggleEditMode} style={styles.editButtonRight}>
