...
export const LIQUID_GLASS_MOMENTS_GLSL = `

uniform float2 u_resolution;
uniform float  u_aspect;
uniform float  u_scale;

uniform float2 u_moments[50];
uniform int    u_momentsLength;
...
// ------------------------------------------------
// SDF + glass helpers (UNCHANGED LOGIC)
// ------------------------------------------------
...
// ------------------------------------------------
// Background sampling (factored once)
// ------------------------------------------------
...
float3 sampleBackground(float2 fragCoord) {
    ...
    float t = clamp(uv.y + 0.5, 0.0, 1.0);
    float3 grad = mix(startColor, endColor, t);

    return mix(grad, grad * 0.6, stripe);
}

// ------------------------------------------------
// Liquid glass lens (shared for ALL dots)
// ------------------------------------------------
...
half4 main(float2 fragCoord) {
    ...
    float2 mainCenter =
        (length(u_lastSelected) < 0.001)
        ? u_resolution * 0.5
        : u_lastSelected * u_resolution;
...
export const LIQUID_GLASS_STRIPES_BG = `
...
uniform float u_aspect;
    ...
    float sdfRect(vec2 c, vec2 s, vec2 p, float r) {
        vec2 q = abs(p - c) - s;
        return length(max(q, 0.0)) + min(max(q.x, q.y), 0.0) - r;
    }
    ...
    vec3 getNormal(vec2 frag, vec2 c, vec2 s, float r, float sd, float thickness) {
        float eps = 2.0;
        float dx = sdfRect(c, s, frag + vec2(eps,0), r) - sdfRect(c, s, frag - vec2(eps,0), r);
        float dy = sdfRect(c, s, frag + vec2(0,eps), r) - sdfRect(c, s, frag - vec2(0,eps), r);
        vec2 g = normalize(vec2(dx, dy));
        float n_cos = max(thickness + sd, 0.0) / thickness;
        float n_sin = sqrt(max(1.0 - n_cos*n_cos, 0.0));
        return normalize(vec3(g * n_cos, n_sin));
    }
...
vec4 main(vec2 fragCoord) {
    ...
    vec3 colorA = gradColor;
    vec3 colorB = gradColor * 0.6;
    vec3 bgColor = mix(colorA, colorB, stripe);

    // ------------------------
    // Glass SDF
    // ------------------------
    vec2 center = u_lastSelected * u_resolution;
    if (length(u_lastSelected) < 0.001) center = u_resolution * 0.5;

    vec2 size = vec2(2.0, 2.0);
    float radius = 64.0;
    float thickness = 10.0;
    float base_height = thickness * 10.0;
    float ior = 1.8;


    float sd = sdfRect(center, size, fragCoord, radius);

    if (sd >= 0.0) return vec4(bgColor, 1.0);


    // ------------------------
    // Refraction + Fresnel
    // ------------------------
    vec3 normal = getNormal(fragCoord, center, size, radius, sd, thickness);
    float h = heightFunc(sd, thickness);

    vec3 incident = vec3(0.0, 0.0, -1.0);
    vec3 refr = refract(incident, normal, 1.0 / ior);

    float travel = (h + base_height) / max(0.001, abs(refr.z));
    vec2 refractedCoord = fragCoord + refr.xy * travel * 3.5;

    // Sample background at refracted coord (recompute uv)
    vec2 uvRefr = refractedCoord / u_resolution;
    uvRefr -= vec2(0.5);
    uvRefr.x *= u_aspect;
    uvRefr.x /= u_scale;

    // Recompute gradient stripes at refracted coord
    float pixelY2 = uvRefr.y * u_resolution.y;
    float stripe2 = step(1.0, mod(pixelY2 / 20.0, 2.0));
    float t2 = clamp(uvRefr.y + 0.5, 0.0, 1.0);
    vec3 gradColor2 = mix(startColor, endColor, t2);
    vec3 colorA2 = gradColor2;
    vec3 colorB2 = gradColor2 * 0.6;
    vec3 refractedColor = mix(colorA2, colorB2, stripe2);

    float fresnel = pow(1.0 - normal.z, 2.0);
    vec3 reflectCol = vec3(fresnel * 0.4);

    vec3 color = mix(refractedColor, reflectCol, fresnel);
...
export const LIQUID_GLASS_GLSL = `
uniform vec2 u_lastSelected;
uniform vec2 u_resolution;
...
vec3 getNormal(
    ...
    float dx = sdfRect(center, size, fragCoord + vec2(eps, 0.0), radius)
             - sdfRect(center, size, fragCoord - vec2(eps, 0.0), radius);

    float dy = sdfRect(center, size, fragCoord + vec2(0.0, eps), radius)
             - sdfRect(center, size, fragCoord - vec2(0.0, eps), radius);
...
vec4 main(vec2 fragCoord) {
    ...
    // Background sample (default)
    vec4 bgColor = u_bg.eval(fragCoord);
    ...
    vec4 refracted = u_bg.eval(refractedCoord);

    // Fresnel-ish reflection
    float fresnel = pow(1.0 - normal.z, 2.0);
    vec3 reflectCol = vec3(fresnel * 0.4);

    vec3 color = mix(refracted.rgb, reflectCol, fresnel);
    color = mix(color, vec3(1.0), 0.00); // .25 for more white
