...
import { packGeckoOnlyProdCompact40, packGeckoOnlyProdCompact_56, packGeckoOnlyProdCompact_56_withLogging} from "./animUtils"; 
...
const GeckoSkia = ({
  ...
  const lastMovementTime = useSharedValue(Date.now()); // Use SharedValue for worklet compatibility
  ...
  const panGesture = Gesture.Pan()
    .onTouchesDown((e) => {
      ...
      lastMovementTime.value = Date.now(); // Reset idle timer on interaction
...
`;
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // ============== CONDITIONAL UPDATES (GATED BY MOVEMENT/IDLE) ==============
      // Track if gecko is currently moving OR if we're within the 10-second hint animation window
      const now = Date.now();
      const timeSinceLastMovement = now - lastMovementTime.value;
      const withinHintWindow = timeSinceLastMovement < IDLE_TIMEOUT_MS;
      ...
      // Update movement timer when gecko is actively moving
      if (leadPoint.current.isMoving || isDragging.value) {
        lastMovementTime.value = now;
      }

      if (shouldUpdateUniforms) {
        // Update hint position (for the animation attached to gecko)
        toShaderSpace_inplace(
          hintRef.current, 
          aspect, 
          gecko_scale, 
