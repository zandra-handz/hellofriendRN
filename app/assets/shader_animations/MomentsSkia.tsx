...
function packVec2Uniform_withRecenter_moments(points, flatArray, num) {
  
  for (let i = 0; i < num; i++) {
   
    if (points[i]) {
      flatArray[i * 2 + 0] = points[i].coord[0] - 0.5;
      flatArray[i * 2 + 1] = points[i].coord[1] - 0.5;
    } else {
      flatArray[i * 2 + 0] = 0.0;
      flatArray[i * 2 + 1] = 0.0;
    }
...
half4 main(vec2 fragCoord) {
    ...
    vec2 selectedUV = u_selected - .5;
    vec2 lastSelectedUV = u_lastSelected - .5;
...
`);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      packVec2Uniform_withRecenter_moments(moments.current.moments, momentsRef.current, moments.current.momentsLength);
    };
    animate();
