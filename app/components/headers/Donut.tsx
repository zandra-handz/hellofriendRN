...
useDerivedValue(() => {
  ...
  const stops = categoryStopsValue.value;
  const n = Math.round(stops[stops.length - 1] || 0);
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (i === 0) {
      arr.push({
        x: RADIUS -labelsSize - 40 - decSize * 10,
        y: RADIUS - labelsSize - 40,
        size: decSize * 10,
        color: colors[0],
      ...
      arr.push({
        x: RADIUS -labelsSize - 40 + offsetX,
        y: RADIUS - labelsSize - 40  + offsetY,
        size: decSize * 10,
