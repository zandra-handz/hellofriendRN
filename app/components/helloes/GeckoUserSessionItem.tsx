...
const GeckoUserSessionItem = ({
  sessionData,
  primaryColor = "orange",
}: Props) => {
  const duration = formatDuration(sessionData.started_on, sessionData.ended_on);
  const friendLabel = sessionData.friend_name ? ` · ${sessionData.friend_name}` : "";
  const label = formatSessionDate(sessionData.started_on);
  const sublabel = `${sessionData.steps} steps · ${Math.round(sessionData.distance)} distance · ${duration}${friendLabel}`;
...
export default GeckoUserSessionItem;