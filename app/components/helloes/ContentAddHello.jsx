...
const ContentAddHello = ({ userId, primaryColor, backgroundColor, friendList }) => {
  ...
  const findFriendById = (id) => friendList?.find((f) => f.id === id) ?? null;
  ...
  useEffect(() => {
    ...
    if (createHelloMutation.isSuccess) {
      ...
      if (pinnedFriend && pinnedFriend !== -1) {
        const pinnedFriendObj = findFriendById(pinnedFriend);
        if (pinnedFriendObj) {
          setToFriend({
            friend: pinnedFriendObj,
            preConditionsMet: true,
          });
        }
        refetchUpcomingHelloes();
        refetchUserStats();
        navigateToMainScreen();
      } else if (upcomingFriend && upcomingFriend !== -1) {
        const upcomingFriendObj = findFriendById(upcomingFriend);
        if (upcomingFriendObj) {
          setToFriend({
            friend: upcomingFriendObj,
            preConditionsMet: true,
          });
        }
        refetchUpcomingHelloes();
  ...
  }, [createHelloMutation.isSuccess, pinnedFriend, upcomingFriend, friendList]);
